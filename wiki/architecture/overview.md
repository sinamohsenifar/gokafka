...
## Request path
1. `gokafka.NewClient` → [[client-lifecycle|connect]]: `Cluster.NegotiateVersions` (ApiVersions) then `Cluster.Refresh` (Metadata).
2. A public call (e.g. `Producer.ProduceSync`) builds a request body via `internal/protocol`, picks the **negotiated** API version, and routes through [[cluster-coordinator|Cluster]] to the right broker.
3. [[transport|Transport]] frames the request, writes it, reads the response, and strips the response header (flexible or not — see [[wire-protocol]]).
